...
const COMPLEXITY_KEYWORDS: Record<TaskComplexity, string[]> = {
  simple: [
    "read", "show", "list", "get", "find one", "simple", "basic",
    "what is", "who is", "where is", "quick"
  ],
  moderate: [
    "search", "analyze", "compare", "explain", "how", "why",
    "summary", "overview", "review", "check"
  ],
  complex: [
    "implement", "create", "build", "design", "architect",
    "optimize", "refactor", "migrate", "integrate", "develop"
  ],
  expert: [
    "debug", "trace", "investigate", "deep dive", "comprehensive",
    "multi-step", "orchestrate", "automate complex", "benchmark"
  ]
};
...
const CAPABILITY_KEYWORDS: Record<ToolCapability, string[]> = {
  ...
  bash_exec: ["run", "execute", "command", "bash", "shell", "npm", "git", "install"],
...
const PARALLEL_INDICATORS = [
  "parallel", "concurrent", "simultaneously", "all at once",
  "multiple", "batch", "many at once", "in parallel"
];

const CONFIRMATION_INDICATORS = [
  "careful", "dangerous", "risky", "delete", "remove", "drop",
  "critical", "important", "confirm", "are you sure", "warning"
];
...
export class Matcher {
  ...
  analyzeTask(query: string, context?: RuleMatchContext): TaskAnalysis {
    ...
    const requiresConfirmation = this.checkConfirmationRequirement(normalizedQuery);
  ...
  private extractKeywords(tokens: string[]): string[] {
    ...
    const stopWords = new Set([
      ...
      "while", "about", "this", "that", "these", "those", "am"
    ]);
  ...
  private checkConfirmationRequirement(query: string): boolean {
    return CONFIRMATION_INDICATORS.some(indicator => query.includes(indicator));
  }
  ...
  matchRules(
    ...
    return detailed.matches.map(r => r.rule);
  }
