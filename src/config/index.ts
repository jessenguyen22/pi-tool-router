  ...
  routingRules: [
    {
      ...
      priority: 10,
      match: {
        queryPatterns: ['edit file', 'modify code', 'change file', 'update file'],
    ...
    },
    {
      ...
      priority: 9,
      match: {
        queryPatterns: ['analyze', 'find function', 'trace', 'search code', 'grep'],
        capabilities: ['code_analysis', 'code_search'],
      },
      preferredTools: ['search', 'read', 'grep'],
    },
    {
      id: 'execution-rule',
    ...
    },
    {
      ...
      priority: 8,
      match: {
        queryPatterns: ['fetch', 'scrape', 'visit url', 'read webpage'],
...
export function validateConfig(config: Partial<RouterConfig>): ValidationResult {
  const errors: string[] = [];

  // Validate strategy types
  const validStrategies: RoutingStrategyType[] = [
    'auto', 'priority', 'cost', 'capability', 'custom',
  ];

  if (config.defaultStrategy && !validStrategies.includes(config.defaultStrategy)) {
  ...
  if (config.routingRules) {
    for (const rule of config.routingRules) {
      ...
      if (typeof rule.priority !== 'number' || rule.priority < 0 || rule.priority > 10) {
        errors.push(`Rule ${rule.id} invalid priority: ${rule.priority} (must be 0-10)`);
      }
  ...
  if (config.analytics) {
    if (config.analytics.retentionDays !== undefined) {
      if (typeof config.analytics.retentionDays !== 'number' || config.analytics.retentionDays < 1) {
...
export class ConfigManager {
  ...
  async addRoutingRule(rule: RoutingRule): Promise<void> {
    const existingIndex = this.config.routingRules.findIndex(r => r.id === rule.id);
    if (existingIndex >= 0) {
  ...
  async updateRoutingRule(ruleId: string, updates: Partial<RoutingRule>): Promise<void> {
    const ruleIndex = this.config.routingRules.findIndex(r => r.id === ruleId);
    if (ruleIndex < 0) {
  ...
  async removeRoutingRule(ruleId: string): Promise<void> {
    ...
    this.config.routingRules = this.config.routingRules.filter(r => r.id !== ruleId);
    await this.update({ routingRules: this.config.routingRules });
  }
   ...
   */
  async toggleStrategy(strategy: RoutingStrategyType, enabled: boolean): Promise<void> {
   ...
   */
  async updateAnalyticsConfig(updates: Partial<AnalyticsConfig>): Promise<void> {
   ...
   */
  on(event: keyof ConfigManagerEvents, callback: ConfigEventCallback): () => void {
    ...
    return () => {
      ...
      this.watchers.set(event, cbs.filter(cb => cb !== callback));
    };
  }
  ...
  private mergeWithDefaults(config: Partial<RouterConfig>): RouterConfig {
    return {
      ...
      routingRules: config.routingRules ?? this.deepClone(DEFAULT_CONFIG.routingRules),
      analytics: {
